                ...
                <div class="project-row" data-aos="fade-up" data-aos-delay="{{ $loop->index * 50 }}">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-3 mb-3 mb-md-0">
                            <img src="{{ asset('storage/' . $project->thumbnail) }}" alt="{{ $project->title ?? '' }}" class="w-100" style="height: 120px; object-fit: cover; filter: grayscale(20%);">
                        </div>
                        @endif
                        <div class="{{ $project->thumbnail ? 'col-md-6' : 'col-md-9' }}">
                            <h5 class="fw-semibold mb-1">{{ $project->title ?? '' }}</h5>
                            <p class="text-muted small mb-0">{{ $project->description ?? '' }}</p>
                        </div>
                        <div class="col-md-3 text-md-end mt-3 mt-md-0">
                            @if($project->live_url)
                            <a href="{{ $project->live_url }}" class="min-link small" target="_blank">View <i class="bi bi-arrow-right"></i></a>
                            @endif
