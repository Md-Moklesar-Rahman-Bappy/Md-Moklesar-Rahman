...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1">Edit Skill</h4>
        <p class="text-muted mb-0">Update <strong>{{ $skill->name }}</strong> details.</p>
    </div>
            ...
            <div class="glass-card mb-4">
                <div class="card-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror"
                                       id="color" name="color"
                                       value="{{ old('color', $skill->color ?? '#6366f1') }}">
                                <input type="text" class="form-control" value="{{ old('color', $skill->color ?? '#6366f1') }}"
                        ...
                        <div class="col-12">
                            <label for="percentage" class="form-label fw-semibold">Proficiency: <span id="percVal">{{ old('percentage', $skill->percentage) }}</span>%</label>
            ...
            <div class="glass-card mb-4">
                <div class="card-body">
                    ...
                    <div class="text-center p-3 rounded" style="background:#f8fafc;">
                        <div class="mb-2" style="font-size:2rem;color:{{ old('color', $skill->color ?? '#6366f1') }};">
                            <i class="{{ old('icon', $skill->icon ?? 'bi bi-check2-circle') }}"></i>
                        </div>
                        <div class="fw-bold">{{ old('name', $skill->name) }}</div>
                        <div class="progress mt-2" style="height:6px;">
                            <div class="progress-bar" style="width:{{ old('percentage', $skill->percentage) }}%;background:{{ old('color', $skill->color ?? '#6366f1') }};"></div>
                        </div>
                        <small class="text-muted">{{ old('percentage', $skill->percentage) }}%</small>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card">
                <div class="card-body">
                    ...
                    <div class="small text-muted mb-2">
                        <i class="bi bi-calendar3 me-2"></i>Created: {{ $skill->created_at->format('M d, Y') }}
                    </div>
                    <div class="small text-muted">
                        <i class="bi bi-clock-history me-2"></i>Updated: {{ $skill->updated_at->format('M d, Y') }}
                    </div>
                </div>
            </div>
        </div>
