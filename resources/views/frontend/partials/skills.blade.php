                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    @foreach($categorySkills as $skill)
                                        <span class="dev-tag">
                                            {{ $skill->name }}
                                            @if($skill->percentage)
                                                <span class="dev-number" style="font-size: 0.65rem;">{{ $skill->percentage }}%</span>
                                            @endif
                                        </span>
                                    @endforeach
                                </div>
                                @if($categorySkills->first()->percentage ?? null)
                                    <div class="mt-3">
                                        @foreach($categorySkills as $skill)
                                            @php $prof = $skill->percentage ?? 0; @endphp
                                            <div class="mb-2">
                                                <div class="d-flex justify-content-between mb-1">
                                                    <span style="font-size: 0.75rem; color: #94a3b8;">{{ $skill->name }}</span>
